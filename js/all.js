...
function chapterload2(cpid, cptitle, croot, cend) {
    ...
    var cpf = $("#cp_img");
    if (cpf.length > 0) {
        $(".cp_tbimg").bind("contextmenu",
        function(e) {
            return false;
        });
    } else {
        $("#showimage").bind("contextmenu",
        function(e) {
            return false;
        });
    }
...
function reseturl(url, ukey) {
    ...
    re.test(u);
    if (RegExp.$3 != "") {
        var reurl = ukey reurl += "-p" + RegExp.$3;
        if (RegExp.$2 != "") {
            reurl += "-pl" + RegExp.$3;
        }
        reurl += "/";
...
function loadimage(arr, index, cid) {
    ...
    function() {
        ...
        if (dm5imgcooke) {
            ...
            for (var i = 0; i < v.length; i++) {
                ...
                if (c[0] == cid && c[1]) {
                    ...
                    if (dm5imgcooke.substr(0, 1) == ",") dm5imgcooke = dm5imgcooke.substring(1, dm5imgcooke.length);
                    if (dm5imgcooke.substr(dm5imgcooke.length - 1, 1) == ",") dm5imgcooke = dm5imgcooke.substr(0, dm5imgcooke.length - 1);
                    break;
                }
            }
        ...
        $.cookie('dm5imgcooke', dm5imgcooke, {
            expires: 1,
            path: '/',
            domain: cookiedm
        });
...
function IsCookieEnabled() {
    ...
    if (v && v == 1) return true;
        ...
        $.cookie('dm5cookieenabletest', 1, {
            expires: 1,
            path: '/',
            domain: cookiedm
        });
        var v = $.cookie('dm5cookieenabletest');
        if (v && v == 1) result = true;
        return result;
    }
}
...
function setnotablefun() {
    ...
    if ($("#dv_fun_jsbt").length > 0) {
        if ($.cookie("fastshow") == null || $.cookie("fastshow") == "true") {
            ...
            $.cookie("fastshow", false, {
                path: "/",
                domain: cookiedm
            });
        }
        $("#dv_fun_jsbt").click(function() {
            if ($(this).attr("val") == "true") {
                DM5_REFRESH = true;
                $(this).attr("val", "");
                $(this).removeClass("cked");
    ...
    if ($("#cbAutosize").length > 0) {
        if ($.cookie("nautosize") != null) {
            $("#cbAutosize").attr("checked", true) setautosize();
        } else {
            autosite = false;
